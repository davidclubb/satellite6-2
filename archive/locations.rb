...
begin
  ...
  # set rest params
  rest_base_url = 'https://192.168.50.130/api/v2/'
  ...
  # get configuration
  loc_config = YAML::load_file('locations.yml')
  ...
  loc_response = build_rest(rest_base_url, "locations", :get, rest_content_type, rest_return_type, rest_api_user, rest_api_password)
  puts loc_response.inspect

  # create locations
  loc_config[:locations].each do |key, attrs|
    log(:info, "Location: #{key}")
    ...
    unless attrs[:parent].nil?
      parent_id = build_rest(rest_base_url, "locations", :get, rest_content_type, rest_return_type, rest_api_user, rest_api_password, { :search => attrs[:parent] } )['results'].first['id']
      log(:info, "Found parent id: #{parent_id}")
      payload[:parent_id] = parent_id
    end

    # make the rest call to create the location
    loc_response = build_rest(rest_base_url, "locations", :post, rest_content_type, rest_return_type, rest_api_user, rest_api_password, payload)
    log(:info, "Inspecting loc_response: #{loc_response.inspect}")
  end
